...
def app():
  ...
  #   app.run()
    ...
    parser.add_argument("server", help="start http server", type=str, nargs="?")
    print(parser.format_help().replace("positional arguments","module/function").replace("[move] ","").replace("[server] ","").replace("[swr2gamma] ","").replace("[wlen2freq] ","").replace("[measurement] ","").replace("[propagation] ","").replace("[propagation]","[module/function]"))
